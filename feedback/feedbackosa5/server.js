...
app.get("/feedback", async (req, res) => {
  try {
    ...
    res.render("feedback", { rows: rows, user: req.session.user });
  } catch (error) {
    console.error(error);
    res.send("Tietokantavirhe");
  }
...
app.get("/support", requireLogin, async (req, res) => {
  try {
    ...
    res.render("support", { rows: rows, user: req.session.user });
  } catch (error) {
    console.error(error);
    res.send("Tietokantavirhe");
  }
  ...
  try {
    ...
    if (status) {
      const [closedStatus] = await pool.query(
        "SELECT id FROM ticket_status WHERE description = 'Closed'",
      );
      let sql, params;
...
app.post("/login", async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(
      "SELECT * FROM system_user WHERE (id = ? OR email = ?) AND admin = 1",
    ...
    const match = await bcrypt.compare(password, user.passwords); // tarkista sarake
    ...
    req.session.user = {
      ...
      admin: user.admin, // tärkeää, jotta admin-tarkistus toimii
    };
